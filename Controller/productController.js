...
const addReview = asyncErrorHandler(async(req,res,next)=>{
    ...
    product.reviews.forEach( ele =>{
        ...
        product.rating = (productRating * reviewLength -ele.rating + Number(rating) ) / (reviewLength);
    ...
    if(!isReviewed) {
        ...
        // update Overall Rating
        product.rating = (productRating * reviewLength + Number(rating)) / (reviewLength+1)
...
const deleteReview = asyncErrorHandler(async(req,res,next)=>{
    ...
    if(isDeleted){
        rating = (product.rating * product.numOfRevies - newRating) / (reviews.length);
...
export {addReview, getAllReview, deleteReview, createProduct, getAllProduct, updateProduct, deleteProduct, getProduct };